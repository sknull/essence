...
import de.visualdigits.essence.util.TraversalHelpers
import de.visualdigits.essence.util.find
import de.visualdigits.essence.words.StopWords

class TextScoreCleaner(private val stopWords: StopWords) : ScoreCleaner {

    override fun clean(element: ScoredElement?): ScoredElement? {
        ...
        addSiblingsToTopNode(topNode)?.let { updatedElement ->
            updatedElement.children().forEach { child ->
                if (!isParagraphOrAnchor(child) &&
                        (NodeHeuristics.hasHighLinkDensity(child) || NodeHeuristics.isTableOrListWithNoParagraphs(child) || !NodeHeuristics.isNodeThresholdMetText(updatedElement, child)) &&
                        child.hasParent()) {
                    child.remove()
                }
            }
    ...
    override fun cleanHtml(element: ScoredElement?): ScoredElement? {
        if (element == null) return null
